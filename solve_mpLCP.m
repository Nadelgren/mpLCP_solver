...
options = optimoptions('fmincon','Algorithm','active-set','Display', 'off','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
options_ = optimoptions('fmincon','Algorithm','active-set','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
options2 = optimoptions('fmincon','Algorithm','sqp','Display', 'off','ConstraintTolerance',con_tol);
options3 = optimoptions('fmincon','Algorithm','interior-point','Display', 'off','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
options2_ = optimoptions('fmincon','Algorithm','sqp','ConstraintTolerance',con_tol);
options3_ = optimoptions('fmincon','Algorithm','interior-point','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
if(svd_nan_inf_flag == 1)
	options = options2;
	options2 = options3;
end
if(increase_fmincon_time == 1)
     options = optimoptions('fmincon','Algorithm','sqp','Display', 'off','MaxIterations',5000,'MaxFunctionEvaluations',10000,'ConstraintTolerance',con_tol);
     options3 = optimoptions('fmincon','Algorithm','interior-point','Display', 'off','MaxIterations',8000,'MaxFunctionEvaluations',16000,'ConstraintTolerance',con_tol);
end
...
%E and H
if(print_stuff)
   disp(sprintf('Building E and H. Line %d',MFileLineNr()));
end
for i = 1:h
%     if(print_stuff)
%         i
        ...
        %j
            if(Z(1,j) ~= 1 && E(1,j) ~= 1 && j ~= i)
                if(H(1,(i-1)*h + j) ~= 1)
                    ...
                    if(use_rand == 1)
                         [sol,fval,exitflag] = fmincon(fun,rand(1,4),A,b,Aeq,beq,lb(1:k+2),ub(1:k+2),nonlcon,options);
%                     elseif(print_stuff)
...
%                              [sol,fval,exitflag] = fmincon(fun,x0,A,b,Aeq,beq,lb(1:k+2),ub(1:k+2),nonlcon,options2_);
                             ...
                             [sol,fval,exitflag] = fmincon(fun,rand(1,4),A,b,Aeq,beq,lb(1:k+2),ub(1:k+2),nonlcon,options2);
                        end
                        %nonlcon(sol)
                        if(exitflag2 ~= exitflag && exitflag2 == 0)
                            exitflag = exitflag2;
                        %else
...
while(size(basis_stack,1) > 0 &&  phase1_iter < phase1_max_iter && etime(clock,t) < max_time)
	...
	if( ismember(phase1_iter,phase2_failed_iters) && svd_nan_inf_flag == 1)
		options = options2;
		options2 = options3;
	else
		options = optimoptions('fmincon','Algorithm','active-set','Display', 'off','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
...
%    fval
   ...
   if(exitflag == 0)
       [sol,fval,exitflag2] = fmincon(fun,sol,A,b,Aeq,beq,lb(1:k+2),ub(1:k+2),nonlcon,options2);
       nlp_s_sol = sol;
...
%                phase2_feas_pt = sol;
               ...
               if(print_stuff)
                   disp(sprintf('Printing F. Line %d',MFileLineNr()));
                   F
               end
...
%                                                             return
       ...
       if(basis_dim(row) == 1)
           %find phase 1 facets
           if(print_stuff)
                disp(sprintf('Calculating constraints satisfied at equality %d',MFileLineNr()));
           end
           min_val = 999999;
           min_index = 0;
           for i = 1:h
               v = abs(subs(RHS(row,i),x,sol));
               if( v < min_val )
                   min_val = v;
                   min_index = i; 
               end
...
%                                             end
           ...
           if(number_new == 0 && less_than_min_fval == 1 && min_fval > 0)
               disp(sprintf('The Problem is Infeasible, exitting!! %d',MFileLineNr())); %%ok<*DSPS>
               E
               return
           end
...
%            end
 ...
 if(min_fval > 0)
       disp(sprintf('The Problem is Infeasible, exitting!! %d',MFileLineNr()));
       E
       return
 end
...
while(size(basis_stack,1) > 0 &&  phase2_iter < phase2_max_iter && etime(clock,t) < max_time)
	...
	if( ismember(phase2_iter,phase2_failed_iters) && svd_nan_inf_flag == 1)
		options = options2;
		options2 = options3;
	else
		options = optimoptions('fmincon','Algorithm','active-set','Display', 'off','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
		options2 = optimoptions('fmincon','Algorithm','sqp','Display', 'off','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);
		options3 = optimoptions('fmincon','Algorithm','interior-point','Display', 'off','MaxIterations',4000,'MaxFunctionEvaluations',8000,'ConstraintTolerance',con_tol);	
	end

   %initialize
   disp(sprintf('Phase 2 iteration: %d. Elapsed time: %.2f sec',phase2_iter,etime(clock,t)));
...
%                            x0 = feas_pt(1:k+1);
                           ...
                           if(exitflag >= 1)
                                 ...
                                 %disp(sprintf('Added basis corresponds to full dimensional region, Line %d',MFileLineNr()));
                                 if(k == 2 && plot_if_k_is_2)
    ...
    %                              temp_RHS
                             ...
                             elseif(exitflag == -2)
                                 ...
                                 basis_dim = [basis_dim;0];
                                 %disp(sprintf('Added basis corresponds to low dimensional region, Line %d',MFileLineNr()));
                             else
                                 feas_pts = [feas_pts;x0];
                                 basis_dim = [basis_dim;0];
%                                 disp(sprintf('Fmincon failed. Exitflag: %d, Line %d',exitflag,MFileLineNr()));
